...
uint8_t write_imu_reg(ICM20948 *imu, uint8_t reg_addr, uint8_t data) {
	...
	uint8_t status = (HAL_SPI_Transmit(imu->spiHandle, TxData, sizeof(TxData), 1) == HAL_OK); // write data to specified register
...
uint8_t read_imu_reg(ICM20948 *imu, uint8_t reg_addr, uint8_t *data) {
	...
	HAL_GPIOWritePin(imu->ncs_pin_bank, imu->ncs_pin, GPIO_PIN_RESET); // pull NCS low for the IMU, selecting it for data transfer
	uint8_t status = (HAL_SPI_TransmitReceive(imu->spiHandle, TxData, RxData, size(RxData), 1) == HAL_OK); // simultaneous write/read operation on the specified register
	HAL_GPIOWritePin(imu->ncs_pin_bank, imu->ncs_pin, GPIO_PIN_SET); // pull NCS high for IMU, de-selecting it for data transfer
...
void write_mag_reg(ICM20948 *imu, uint8_t reg_addr, uint8_t data) {
	...
	write_imu_reg(imu, USER_BANK_SEL, USER_BANK_3);
...
uint8_t read_mag_reg(ICM20948 *imu, uint8_t reg_addr) {
	...
	write_imu_reg(imu, I2C_SLV0_ADDR, (0x0C | 0x80));
	// writing the desired register address within the AK09166 to begin data transmission with
	write_imu_reg(imu, I2C_SLV0_REG, reg_addr);

	write_imu_reg(imu, I2C_SLV0_DO, 0xFF); // read

	// writing a value of 0x00 (00000011) to USER_BANK_SEL register
	// select User Bank 0 to access data out registers for the AK09916
	write_imu_reg(imu, USER_BANK_SEL, USER_BANK_0);

	// AK09916 data output gets put to EXT_SLV_SENS_DATA_00
	read_imu_reg(imu, EXT_SLV_SENS_DATA_00, &data);
	return data;
}
...
uint8_t who_am_i(ICM20948 *imu) {
	...
	read_imu_reg(imu, WHO_AM_I, &imu_d);
...
uint8_t read_accel_vec(imu *imu) {
	...
	uint8_t status = read_imu_reg(imu, 0x2D, RxData);
...
uint8_t read_gyro_vec(imu *imu) {
	...
	// hence why we shift the MSB left by 8 bits
	// the bitwise "or" operator is used to combine the MSB and LSB to create a 16 bit integer
	uint8_t status = read_imu_reg(imu, 0x31, RxData);
...
void read_mag_vec(imu *imu) {
	...
	uint8_t status = read_imu_reg(imu, EXT_SLV_SENS_DATA_00, mag_buffer);
	...
	read_mag_reg(imu, 0x18); // reading register STATUS_2 is required as end register to stop reading
...
void calibrate_gyro(imu *imu) {
	...
	write_imu_reg(imu, XG_OFFS_USRH, gyro_offset[0]);
	write_imu_reg(imu, XG_OFFS_USRL, gyro_offset[1]);
	write_imu_reg(imu, YG_OFFS_USRH, gyro_offset[2]);
	write_imu_reg(imu, YG_OFFS_USRL, gyro_offset[3]);
	write_imu_reg(imu, ZG_OFFS_USRH, gyro_offset[4]);
	write_imu_reg(imu, ZG_OFFS_USRL, gyro_offset[5]);
...
void calibrate_accel(imu *imu) {
	...
	// write offsets to corresponding registers
	// these registers store offset values and automatically use them to compensate for bias during measurements
	write_imu_reg(imu, XA_OFFS_USRH, accel_offset[0]);
	write_imu_reg(imu, XA_OFFS_USRL, accel_offset[1]);
	write_imu_reg(imu, YA_OFFS_USRH, accel_offset[2]);
	write_imu_reg(imu, YA_OFFS_USRL, accel_offset[3]);
	write_imu_reg(imu, ZA_OFFS_USRH, accel_offset[4]);
	write_imu_reg(imu, ZA_OFFS_USRL, accel_offset[5]);
}
