...
/*
 * @brief Write as single byte of data to a specified IMU register
...
uint8_t write_imu_single(uint8_t reg_addr, uint8_t TxData) {
	...
	uint8_t status = SPI_transmit(TxBuffer, 2, 1); // write data to specified register
...
uint8_t write_imu_multiple(uint8_t reg_addr, uint8_t TxData, uint8_t size) {
	...
	peripheral_select(ICM20948_NCS); // pull NCS low for the IMU, selecting it for data transfer
	uint8_t status = SPI_transmit(TxBuffer, size+1, 1); // burst write data to specified register
	peripheral_deselect(ICM20948_NCS); // pull NCS high for IMU, de-selecting it for data transfer
...
uint8_t read_imu_single(uint8_t reg_addr, uint8_t *RxBuffer) {
	...
	peripheral_select(ICM20948_NCS); // pull NCS low for the IMU, selecting it for data transfer
	SPI_transmit(&TxBuffer, 1, 1); // write the register address to read from alongside the bit signaling a read operation
	uint8_t status = SPI_receive(RxBuffer, 1, 1); // receive byte into RxBuffer
	peripheral_deselect(ICM20948_NCS); // pull NCS high for IMU, de-selecting it for data transfer
...
/*
 * @brief Burst read a multiple bytes of data from a specified starting IMU register
 * @params the desired ICM20948 register address to read from
 * @params RxBuffer: a pointer to the data buffer the read data is stored at
 * @params size: size of the data buffer to be transmitted
 * @return Status of the data transfer
...
uint8_t read_imu_multiple(uint8_t reg_addr, uint8_t *RxBuffer, uint8_t size) {
	...
	peripheral_select(ICM20948_NCS); // pull NCS low for the IMU, selecting it for data transfer
	SPI_transmit(&TxBuffer, 1, 1); // write the register address to read from alongside the bit signaling a read operation
	uint8_t status = SPI_receive(RxBuffer, size, 1); // receive byte into RxBuffer
	peripheral_deselect(ICM20948_NCS); // pull NCS high for IMU, de-selecting it for data transfer
